...
const SendVerificationForm: React.FC<Props> = (props) => {
  ...
  const sendVerification = async (data: FormValues) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_API_PATH}/user/verifycode`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="animate-fadeIn container mx-auto w-2/3 rounded-sm shadow-md bg-base-200">
      <form onSubmit={handleSubmit(sendVerification)} className="p-2">
        ...
        <div className="flex px-10">
          <button
            className="text-lg w-full mt-8 px-2 py-0.5 font-outfit text-base-content hover:text-stone-700"
            onClick={() => {
